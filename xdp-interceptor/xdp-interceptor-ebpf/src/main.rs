...
fn get_raw_packet(ctx: &XdpContext) -> Result<*const [u8], ExecutionError> {
    ...
    if let Some(new_end) = start.checked_add(len) {
        if new_end > end {
            return Err(ExecutionError::PointerOverflow);
        }
    } else {
    ...
    let data_buffer: *const u8 = start as usize as *const u8;
    Ok( unsafe{core::slice::from_raw_parts(data_buffer, len)} )

    // if raw_packet.len() < 20 || computed_ihl < 20 {
    //     info!(ctx, "Packet too small");
    //     return Err(ExecutionError::PacketTooSmall);
    // }
    //
    // if raw_packet.len() < (computed_ihl as usize) || (total_length as usize) < computed_ihl {
    //     info!(ctx, "Packet malformed");
    //     return Err(ExecutionError::PacketMalformed);
    // }
    //
    // if version != 4 {
    //     info!(ctx, "Packet malformed");
...
fn try_xdp_interceptor(ctx: XdpContext) -> Result<u32, ExecutionError> {
    ...
    let data: *const [u8] = match get_raw_packet(&ctx) {
        Ok(data) => data,
        Err(_) => {
    ...
    if let Some(data) = unsafe {data.as_ref()}{
        if data.len() > 0{
            ...
            info!(&ctx, "length: {}", data.len());
            if let Ok(s) = core::str::from_utf8(data) {
                info!(&ctx, "problem with info!!!")
                // info!(&ctx, "received data: {}", s);
            } else{
                info!(&ctx, "problem with info!");
            }
        }
    } else{
        info!(&ctx, "problem problem problem");
    }
    
    // let string_data = unsafe { core::str::from_utf8_unchecked(&*data) };
    // info!(&ctx, "Packet");
    //print packet
    // info!(&ctx, "Packet: {}", string_data);
