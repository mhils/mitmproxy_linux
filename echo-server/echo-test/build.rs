...
/// This crate has a runtime dependency on artifacts produced by the `echo-test-ebpf` crate.
...
/// crate's undeclared dependency on artifacts from `echo-test-ebpf` would cause build (and
...
/// performance; an environment variable is used to replace true builds of `echo-test-ebpf`
/// with stubs to preserve the property that code generation and linking (in
/// `echo-test-ebpf`) do not occur on metadata-only actions such as `cargo check` or `cargo
...
fn main() {
    ...
    let ebpf_package = packages
        .into_iter()
        .find(|Package { name, .. }| name == "echo-test-ebpf")
        .unwrap();
    ...
    if build_ebpf {
        ...
        // We have a build-dependency on `echo-test-ebpf`, so cargo will automatically rebuild us
        // if `echo-test-ebpf`'s *library* target or any of its dependencies change. Since we
        // depend on `echo-test-ebpf`'s *binary* targets, that only gets us half of the way. This
        ...
        let ebpf_target_dir = out_dir.join("echo-test-ebpf");
